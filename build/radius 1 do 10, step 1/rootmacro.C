void rootmacro(){
    ...
    Molliere_graph -> GetXaxis() -> SetTitle ("radius (cm)");
    Molliere_graph -> GetYaxis() -> SetTitle ("frac");

    auto Molliere_fun = new TF1 ("Molliere function", "-1/(x*[0])+1", 0, 10);
    Molliere_fun->SetParameter(0, 3.5);
    Molliere_graph->Fit(Molliere_fun);

    auto line = new TLine(0, 0.9, 11, 0.9);
